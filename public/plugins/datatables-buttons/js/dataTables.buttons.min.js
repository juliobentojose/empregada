...
*/
(function(d) { "function" === typeof define && define.amd ? define(["jquery", "datatables.net"], function(z) { return d(z, window, document) }) : "object" === typeof exports ? module.exports = function(z, B) { z || (z = window);
        B && B.fn.dataTable || (B = require("datatables.net")(z, B).$); return d(B, z, z.document) } : d(jQuery, window, document) })(function(d, z, B, p) {
    ...
    d.extend(x.prototype, {
        ...
        _constructor: function() {
            ...
            d("body").on("keyup." + this.s.namespace, function(g) { if (!B.activeElement || B.activeElement === B.body) { var l = String.fromCharCode(g.keyCode).toLowerCase(); - 1 !== a.s.listenKeys.toLowerCase().indexOf(l) && a._keypress(l, g) } })
        },
        ...
        _buildButton: function(a, b, c, e) {
            var h = this.c.dom.button,
                ...
                r = this.s.dt,
                q = function(w) { return "function" === typeof w ? w(r, k, a) : w };
            ...
            if (a.hasOwnProperty("html")) var k = d(a.html);
                ...
                var t = function(w, D, F, G) { G.action.call(D.button(F), w, D, F, G);
                    d(D.table().node()).triggerHandler("buttons-action.dt", [D.button(F), D, F, G]) };
                g = a.tag || h.tag;
                var y = a.clickBlurs === p ? !0 : a.clickBlurs;
                k = d("<" + g + "/>").addClass(h.className).addClass(e ? this.c.dom.splitDropdownButton.className : "").attr("tabindex", this.s.dt.settings()[0].iTabIndex).attr("aria-controls", this.s.dt.table().node().id).on("click.dtb", function(w) { w.preventDefault();!k.hasClass(h.disabled) && a.action && t(w, r, k, a);
                    y && k.trigger("blur") }).on("keypress.dtb",
                    function(w) { 13 === w.keyCode && (w.preventDefault(), !k.hasClass(h.disabled) && a.action && t(w, r, k, a)) });
            ...
            if (c) {
                ...
                this._addKey(v);
                var E = function(w, D, F, G) { C.split.action.call(D.button(d("div.dt-btn-split-wrapper")[0]), w, D, F, G);
                        d(D.table().node()).triggerHandler("buttons-action.dt", [D.button(F), D, F, G]);
                    ...
                    A = d('<button class="' + this.c.dom.splitDropdown.className + ' dt-button"><span class="dt-btn-split-drop-arrow">' + this.c.dom.splitDropdown.text + "</span></button>").on("click.dtb", function(w) {
                        ...
                        y && A.trigger("blur")
                    }).on("keypress.dtb", function(w) { 13 === w.keyCode && (w.preventDefault(), A.hasClass(h.disabled) || E(w, r, A, v)) });
        ...
        _popover: function(a, b, c, e) {
            ...
            if (!1 ===
                ...
                } else r = function() { var E = d(z).height() / 2,
                        A = m.height() / 2;
                    A > E && (A = E);
                    m.css("marginTop", -1 * A) }, r(), d(z).on("resize.dtb-collection", function() { r() });
                ...
                f.autoClose && setTimeout(function() { b.on("buttons-action.b-internal", function(E, A, w, D) { D[0] !== g[0] && l() }) }, 0);
                d(m).trigger("buttons-popover.dt");
                b.on("destroy", l);
                setTimeout(function() {
                    h = !1;
                    d("body").on("click.dtb-collection",
                        function(E) { if (!h) { var A = d.fn.addBack ? "addBack" : "andSelf",
                                    w = d(E.target).parent()[0];
                                (!d(E.target).parents()[A]().filter(a).length && !d(w).hasClass("dt-buttons") || d(E.target).hasClass("dt-button-background")) && l() } }).on("keyup.dtb-collection", function(E) { 27 === E.keyCode && l() })
                }, 0)
            }
        }
    });
    ...
    x.instanceSelector = function(a,
        b) { if (a === p || null === a) return d.map(b, function(f) { return f.inst }); var c = [],
            e = d.map(b, function(f) { return f.name }),
            h = function(f) { if (Array.isArray(f))
    ...
    x.buttonSelector = function(a, b) {
        for (var c = [], e = function(l, m, r) {
                ...
                if (Array.isArray(l) || l instanceof d)
                    ...
                    for (l = l.replace(":name", ""), n = 0, r = q.length; n < r; n++) q[n].name === l && c.push({ inst: m, node: q[n].node });
    ...
    x.version = "2.2.2";
    d.extend(C, {
        collection: { text: function(a) { return a.i18n("buttons.collection", "Collection") }, className: "buttons-collection", closeButton: !1, init: function(a, b, c) { b.attr("aria-expanded", !1) }, action: function(a, b, c, e) { e._collection.parents("body").length ? this.popover(!1, e) : this.popover(e._collection, e) }, attr: { "aria-haspopup": !0 } },
        split: {
            text: function(a) { return a.i18n("buttons.split", "Split") },
            className: "buttons-split",
            closeButton: !1,
            init: function(a, b, c) { return b.attr("aria-expanded", !1) },
            action: function(a, b, c, e) { this.popover(e._collection, e) },
            attr: { "aria-haspopup": !0 }
        },
        copy: function(a, b) { if (C.copyHtml5) return "copyHtml5" },
        csv: function(a, b) { if (C.csvHtml5 && C.csvHtml5.available(a, b)) return "csvHtml5" },
        excel: function(a, b) { if (C.excelHtml5 && C.excelHtml5.available(a, b)) return "excelHtml5" },
        pdf: function(a, b) { if (C.pdfHtml5 && C.pdfHtml5.available(a, b)) return "pdfHtml5" },
        pageLength: function(a) {
            ...
            return {
                extend: "collection",
                text: function(f) { return f.i18n("buttons.pageLength", { "-1": "Show all rows", _: "Show %d rows" }, f.page.len()) },
                ...
                buttons: d.map(b, function(f, g) {
                    return {
                        ...
                        action: function(l, m) { m.page.len(f).draw() },
                        init: function(l, m, r) { var q = this;
                ...
                destroy: function(f, g, l) { f.off("length.dt" + l.namespace) }
            }
        },
    ...
    u.Api.register("buttons()", function(a, b) {
        ...
        var c = this.iterator(!0, "table", function(e) { if (e._buttons) return x.buttonSelector(x.instanceSelector(a, e._buttons), b) }, !0);
        c._groupSelector =
            a;
        return c
    });
    ...
    u.Api.registerPlural("buttons().active()", "button().active()", function(a) { return a === p ? this.map(function(b) { return b.inst.active(b.node) }) : this.each(function(b) { b.inst.active(b.node, a) }) });
    ...
    u.Api.registerPlural("buttons().collectionRebuild()",
        ...
        function(a) { return this.each(function(b) { for (var c = 0; c < a.length; c++) "object" === typeof a[c] && (a[c].parentConf = b);
    ...
    u.Api.registerPlural("buttons().nodes()", "button().node()", function() { var a = d();
    ...
    u.Api.registerPlural("buttons().trigger()", "button().trigger()", function() { return this.each(function(a) { a.inst.node(a.node).trigger("click") }) });
    ...
    u.Api.register("buttons().container()", function() { return this.containers().eq(0) });
    ...
    u.Api.register("buttons.info()", function(a, b, c) {
        ...
        c !== p && 0 !== c && (H = setTimeout(function() { e.buttons.info(!1) }, c));
    ...
    var K = function(a) {
        ...
        N = function(a, b, c) { b = K(b); if (null === b) return null;
        ...
        Q = function(a, b) {
            var c = d.extend(!0, {}, {
                ...
                trim: !0,
                format: {
                    header: function(t) { return x.stripData(t, c) },
                    footer: function(t) { return x.stripData(t, c) },
                    body: function(t) {
            ...
            b = a.columns(c.columns).indexes().map(function(t) { var y = a.column(t).header(); return c.format.header(y.innerHTML, t, y) }).toArray();
            ...
            f = f.nodes().toArray();
            for (var g = b.length, l = [], m = 0, r = 0, q = 0 < g ? h.length / g : 0; r < q; r++) { for (var n = [g], k = 0; k < g; k++) n[k] = c.format.body(h[m], r, k, f[m]), m++;
                l[r] = n }
    ...
    d.fn.DataTable.Buttons = x;
    d(B).on("init.dt plugin-init.dt", function(a, b) { "dt" === a.namespace && (a = b.oInit.buttons || u.defaults.buttons) && !b._buttons && (new x(b, a)).container() });
    u.ext.feature.push({
        fnInit: L,
        cFeature: "B"
    });
